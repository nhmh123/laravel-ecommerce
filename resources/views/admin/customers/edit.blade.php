...
@section('content')
    <div class="max-w-2xl">
        <div class="mb-4">
            <h2 class="text-xl font-medium">Edit customer</h2>
        ...
        @isset($customer)
            <form method="POST" action="{{ url('/admin/customers/' . $customer->id) }}" class="space-y-4">
                @csrf
                @method('PUT')
        ...
        @else
            <div class="rounded-sm border border-[#e3e3e0] dark:border-[#3E3E3A] bg-white dark:bg-[#161615] p-4 text-sm">
                No customer loaded. Pass a `$customer` model from your controller.
            </div>
        @endisset
    </div>
@endsection
